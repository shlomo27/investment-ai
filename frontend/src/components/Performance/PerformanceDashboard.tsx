...
export default function PerformanceDashboard({ isHebrew = true }: Props) {
  ...
  if (loading) return <div style={{ padding: 24, color: "#888" }}>{t("Loading performance...", "טוען ביצועים...")}</div>;
    ...
    <div style={{ padding: 20 }}>
      <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
        {(["summary", "history"] as const).map((tb) => (
          ...
          >
            {tb === "summary"
              ? t("Performance Summary", "סיכום ביצועים")
              : t("History", "היסטוריה")}
          </button>
        ))}
      </div>
        ...
        <div>
          {summary.total_tracked === 0 ? (
            <div style={{ color: "#888", textAlign: "center", padding: 40 }}>
              {t("No tracked outcomes yet (appear 30 days after approval)", "אין עדיין המלצות במעקב (יופיעו לאחר 30 יום מיום האישור)")}
            </div>
          ) : (
            <>
              {/* KPI cards */}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 16, marginBottom: 24 }}>
                {[
                  { label: t("Total Tracked", "סה\"כ במעקב"), value: summary.total_tracked, color: "#fff" },
                  { label: t("Win Rate", "אחוז הצלחה"), value: `${summary.win_rate_pct}%`, color: "#22c55e" },
                  { label: t("Avg Return", "תשואה ממוצעת"), value: `${summary.avg_return_pct > 0 ? "+" : ""}${summary.avg_return_pct}%`, color: returnColor(summary.avg_return_pct) },
                  { label: t("vs S&P 500", "alpha vs S&P500"), value: `${summary.avg_vs_market_pct > 0 ? "+" : ""}${summary.avg_vs_market_pct}%`, color: returnColor(summary.avg_vs_market_pct) },
                  { label: t("Wins", "ניצחונות"), value: summary.win_count, color: "#22c55e" },
                  { label: t("Losses", "הפסדים"), value: summary.loss_count, color: "#ef4444" },
                ].map((kpi) => (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
                {summary.best_trade && (
                  <div style={{ background: "#0f2d1c", border: "1px solid #22c55e", borderRadius: 12, padding: 16 }}>
                    <div style={{ color: "#22c55e", fontWeight: 600, marginBottom: 8 }}>🏆 {t("Best Trade", "עסקה מוצלחת ביותר")}</div>
                ...
                {summary.worst_trade && (
                  <div style={{ background: "#2d0f0f", border: "1px solid #ef4444", borderRadius: 12, padding: 16 }}>
                    <div style={{ color: "#ef4444", fontWeight: 600, marginBottom: 8 }}>📉 {t("Worst Trade", "עסקה גרועה ביותר")}</div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ color: "#64748b", borderBottom: "1px solid #334155" }}>
                {[t("Symbol", "סימול"), t("Type", "המלצה"), t("Entry", "מחיר כניסה"),
                  t("Exit", "מחיר יציאה"), t("Return", "תשואה"), t("vs Mkt", "vs שוק"), t("Result", "תוצאה")]
            ...
            <tbody>
              {history.map((r) => (
                <tr key={r.id} style={{ borderBottom: "1px solid #1e293b" }}>
                  ...
                  <td style={{ padding: "8px 12px", color: "#94a3b8" }}>{r.outcome_price ? `$${r.outcome_price.toFixed(2)}` : t("Pending", "ממתין")}</td>
                  ...
                  <td style={{ padding: "8px 12px" }}>
                    <span style={{ background: resultColor(r.outcome_result) + "22", color: resultColor(r.outcome_result), borderRadius: 6, padding: "2px 8px", fontSize: 12 }}>
                      {r.outcome_result || t("PENDING", "ממתין")}
                    </span>
                  </td>
                </tr>
              ))}
              {history.length === 0 && (
                <tr><td colSpan={7} style={{ textAlign: "center", color: "#64748b", padding: 40 }}>{t("No data yet", "אין נתונים")}</td></tr>
              )}
            </tbody>
          </table>
        </div>
